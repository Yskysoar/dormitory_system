...
public class AccountController {
    ...
    public ModelAndView login(AccountForm accountForm, HttpSession httpSession) {
        ...
        if (Objects.equals(accountForm.getAction(), "login")) {
        ...
        } else if (Objects.equals(accountForm.getAction(), "register")) {
            AccountDto accountDto = this.accountService.register(accountForm);
            switch (accountDto.getCode()) {//根据状态码分支执行操作
                case 1: {//状态：注册成功
                    switch (accountForm.getType()) {//根据管理员类型分支执行操作
                        case "dormitoryAdmin": {
                ...
                case -3: {//用户名已存在
                    modelAndView.setViewName("login");
                    modelAndView.addObject("usernameRepeat", "用户名已存在");//返回当前状态
                }
            }
        }
        return modelAndView;
